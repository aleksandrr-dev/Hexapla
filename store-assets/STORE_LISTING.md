...
Audio is saved for offline listening, or turn on streaming to save space.
...
Аудио сохраняется для прослушивания офлайн, либо включите потоковый режим.
...
klare Vertonung. Audio wird offline gespeichert oder kann gestreamt werden.
...
2026-07-16/17). Totals used in the listing bodies below — **34 translations
across 29 languages** (updated 2026-07-20 when the Persian Martyn NT `mrt`
shipped in 1.6.0, +1 translation / +1 language) — come from the
`translations` list in `app/src/main/java/com/aleks/hexapla/Bible.kt`
(35 `Translation` registrations; the Simplified/Traditional Chinese Union
Version pair, `cus` and `cuv`, is one translation in two scripts, netting
34) cross-checked
...
previously represented by any other entry, so it's a straight +1/+1). Then
the Persian Martyn NT `mrt` (2026-07-20, 1.6.0) added another straight
+1/+1 → the current 34 translations / 29 languages. All 11
